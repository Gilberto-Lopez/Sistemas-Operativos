...
//void print(void* begin, void* end);
...
start_process (void *file_name_)
{
  ...
  /*Practica6*/
  struct thread *act = thread_current();
  act->father->exec_status = success;
...
process_wait (tid_t child_tid UNUSED)
{
  ...
  sema_down(&thread_current()->exec_sem);
  list_remove(&luke->son_elem);
  return luke->exit_stat;
}
...
process_exit (void)
{
  ...
  for(sons = list_begin(&cur->father->sons); sons != list_end(&cur->father->sons); sons = list_next(sons)){
    struct son *leia = list_entry(sons,struct son,son_elem);
    if(leia->id == cur->tid){
      luke = leia; //incest jeje SW:Empire Strikes Back
    }
  }
  if(cur->father != NULL){
